...
var measureEvent = function(value, type, url){
    ...
    switch (type){
        ...
        case 'ready':
            ...
            if (readyTime == null && startTime && url == tabs.activeTab.url){
...
pageMod.PageMod({
    include: "*",
    contentScript: 'self.port.emit("measureEvent", new Date().getTime(), "start");',
...
pageMod.PageMod({
    include: "*",
    contentScript: 'self.port.emit("measureEvent", new Date().getTime(), "ready", window.location.href);',
